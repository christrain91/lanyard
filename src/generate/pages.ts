...
function getLanyardFiles () {
  const fronts = getFilePaths('/output/lanyards')
  const backs = getFilePaths('/output/backs')

  return fronts.map((front, index) => ({ front, back: backs[index]}))
}

function generateSinglePage(name: string, lanyards: { front: string, back: string}[], positions: [number, number][]) : Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    lanyards.forEach((lanyard, index) => {
      ...
      placeImage({ gm: process,  path: lanyard.back, position: backPosition, size: LANYARD })
    })
...
function getPositionsForLanyards (lanyardDimensions: [number, number] = LANYARD) : [number, number][] {
